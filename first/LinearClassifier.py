...
if __name__ == "__main__":
    ...
    loss = 0
    for i in xrange(Xtr_rows.shape[0]):
        loss +=  L_i_vectorized(Xtr_rows[i], Ytr_rows[i], W)
    print (loss)

    print L(Xtr_rows, Ytr_rows, W)

    Xtr_image = Xtr_rows.reshape(Xtr_rows.shape[0],  32, 32, 3)
    print Xtr_image[1]
    imgplot = plt.imshow(Xtr_image[1])
    plt.show()
